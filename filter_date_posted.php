<head>
  <title>Filter by Date Posted</title>
...
if($_SERVER["REQUEST_METHOD"]=="POST"){ //if I came from a post request
  ...
  $filtering_date_posted=mysqli_query($sql_connection_var, "SELECT * FROM sales_items ORDER BY date_posted DESC");
  ...
  while($printing_table=mysqli_fetch_array($filtering_date_posted)){
    echo("<tr>
...
echo("<h2>Listing Filtered by Date Posted (Newest to Oldest)</h2>");
echo("</table>");
}
 ?>
 <br>
